...
def get_all_links(last_page: int) -> list:
    ...
    for page in tqdm(range(1, last_page + 1), desc="PARSER URLS: "):
        ...
        raw_links = soup.find_all(
            "a",
            class_="z-1 absolute top-0 left-0 w-full h-full cursor-pointer",
            href=True,
        )
    ...
    for link in tqdm(links, desc="parsing data: "):
        ...
        try:
            title = s.find(
                "h1",
                {
                    "class": "order-1 mb-0.5 md:-order-2 md:mb-4 block w-full !inline-block lg:text-h1Lg text-h1 font-raleway font-bold flex items-center"
                },
            ).text
        ...
        try:
            description = s.find(
                "div", class_=["description_wrapper__tlUQE"]).text
        except AttributeError:
            description = ""
        # pprint(description)
        ...
        try:
            image = s.find(
                "div", class_="absolute inset-0").find_all("img")[1]["src"]
        except AttributeError:
            image = ""
        # print(image)
        ...
        for param in raw_params:
            ...
            if key not in PARAM_PATTERN:
                continue
            value = param.find(["p", "a"]).text.replace(
                "г. ", "").replace(" м²", "")
            flat[key] = value
