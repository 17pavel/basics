...
def create_db():
    ...
    cur = conn.cursor()
    cur.execute("DROP TABLE IF EXISTS flats_last")
    cur.execute(
        """CREATE TABLE IF NOT EXISTS flats_last (
...
def insert_db(flats: dict):
    ...
    for flat in flats.values():
        ...
        query = f"INSERT INTO flats_last ({col}) VALUES ({insert})"
...
def get_flats_from_db():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT flat_id FROM flats EXCEPT SELECT flat_id FROM flats_last")
    flats = cur.fetchall()
    print(flats)
